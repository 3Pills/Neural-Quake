...
nextmsg:
		...
		if (ret == -1)
		{
			Dedicated_Printf ("SV_ReadClientMessage: NET_GetMessage failed\n");
			return false;
		}
		...
		while (1)
		{
			...
			default:
				Dedicated_Printf ("SV_ReadClientMessage: unknown command char\n");
				return false;
			...
			case clc_stringcmd:
...

				{
					static const char * valid_cmds[] = {
						...
						"qcexec", "say", "say_team", "spawn", "status", "tell",
						NULL
					};
					const char **cur;
...
==================
*/
void SV_RunClients (double frametime) // MH had this as a float, not double?
{
	int				i;
