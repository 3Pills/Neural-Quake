...
void CalcGunAngle (void)
{
	...
	cl.viewent.angles[ROLL] -= v_idlescale.value * sin(host_frametime_ * v_iroll_cycle.value) * v_iroll_level.value;
	cl.viewent.angles[PITCH] -= v_idlescale.value * sin(host_frametime_ * v_ipitch_cycle.value) * v_ipitch_level.value;
	cl.viewent.angles[YAW] -= v_idlescale.value * sin(host_frametime_ * v_iyaw_cycle.value) * v_iyaw_level.value;
}
...
void View_CalcViewRoll (void)
{
	...
	side = Math_CalcRoll (cl_entities[cl.viewentity].angles, cl.velocity, cl_rollangle.value, cl_rollspeed.value);
