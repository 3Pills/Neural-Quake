...
void Player_IPv4_List_Update (void)
{
	...
	for (n = 0, client = svs.clients ; n < svs.maxclients ; n++, client++)
	{
		const char *unvalidated_ipstring = NET_QSocketGetAddressString(client->netconnection);
...
void Admin_Game_Files_List_Update_Server (void)
{
	...
	for (i = 0, s = sv.sound_precache + 1; *s; s++, i++)
		if (*s)
			List_Add_No_Case_To_Lower (&rawlist, va("soundz/%s", *s) ); // We are in main thread.  And threads never use va
