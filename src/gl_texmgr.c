...
void TexMgr_Imagedump_f (void)
{
	...
	for (glt = active_gltextures; glt; glt=glt->next)
	{
		...
		{
			buffer = (unsigned *) malloc(glt->width*glt->height*4);
...
// exception of making color 255 black with no alpha.
	...
	memcpy(d_8to24table_conchars, vid.d_8to24table, 256*4);
...
void TexMgr_RecalcWarpImageSize (void)
{
	...
	if (oldsize && gl_warpimagesize == oldsize)
		return;
...
#ifdef TEXGAMMA
	unsigned int * fastcolor;
	if (fastpal_rgb4)
	{
		int pels = glt->width * glt->height;
		int i;
		unsigned int rgb_pixel, alpha, checkpix;
		byte red, green, blue;
		...
		for (i = 0; i < pels; i ++)
		{
			rgb_pixel  = data[i] & 0xFFFFFF; // Remove alpha from the equation

			if (rgb_pixel == 0)
				continue;

			alpha = data[i] & 0xFF000000;
...
#endif
			checkpix = ((unsigned)red << 0)
				+ ((unsigned)green << 8)
				+ ((unsigned)blue << 16);

			if (checkpix != rgb_pixel)
				i = i;


			if (fastpal_rgb4[rgb_pixel])
			{
				// Fast evaluation
				data[i] = alpha | fastpal_rgb4[rgb_pixel];
				continue;
			}

			// Fast evaluation failed so create it

			pxelbytes = (byte*)&data[i];

			if (pxelbytes != &data[i])
				i=i;

			pxelbytes[0] /*red*/ = texgamma[pxelbytes[0]];
			pxelbytes[1] /*blue*/ = texgamma[pxelbytes[1]];
			pxelbytes[2] /*green*/ = texgamma[pxelbytes[2]];
			pxelbytes = (byte*)&fastpal_rgb4[rgb_pixel];
			pxelbytes[0] = ((byte*)&data[i])[0];
			pxelbytes[1] = ((byte*)&data[i])[1];
			pxelbytes[2] = ((byte*)&data[i])[2];

			// Loop item runs again, this time fast pal is available.
			i --;
		}
	}
#endif //  TEXGAMMA
...
/*
we only load textures at level load and during init.  Never happens on the fly except for skybox command
we would need to generate warp images.
we will have to stagger the reload.

Events -- alt enter*/
void TexMgr_GammaReload (cbool generate_new_texslot)
{
	gltexture_t *glt;
	int count;

	Image_Quick_Palette_RGBA_Alloc (); // Initialize quick palette
	Image_Build_Gamma_Table (vid_gamma.value, 1, texgamma);
	...
	for (glt = active_gltextures, count = 0; glt; glt = glt->next, count ++)
	{
		...
		VID_Local_Set_Window_Caption (glt->name);
		TexMgr_ReloadImage (glt, -1, -1);
	}
	...
	Image_Quick_Palette_RGBA_Free (); // Shutdown quick palette
}
#endif // TEXGAMMA
