...
// byte swap the lumps
	...
	pr_imp12_override = false;

	if (!pr_handles_imp12 && vm_imp12hack.value && vm_imp12hack.value < 2 && COM_ListMatch (vm_imp12hack_exceptions.string, gamedir_shortname()) )
	{
		Con_DPrintf ("Mod \"%s\" is on impulse 12 exception list\n", gamedir_shortname());
	}
	else
	{
		pr_imp12_override = vm_imp12hack.value >=2 || ( !pr_handles_imp12 && vm_imp12hack.value) ;
			 if (!pr_handles_imp12 && !pr_imp12_override)  Con_Warning ("Progs.dat does not appear to support impulse 12\n");
		else if (!pr_handles_imp12 &&  pr_imp12_override)  Con_Warning ("Overriding missing impulse 12 support\n");
		else if ( pr_handles_imp12 &&  pr_imp12_override)  Con_Warning ("Overriding existing impulse 12 support\n");
		else if ( pr_handles_imp12 && !pr_imp12_override)  Con_DPrintf ("Mod appears to support impulse 12\n");
	}
