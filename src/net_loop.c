...
void Loop_SearchForHosts (cbool xmit)
{
	if (!sv.active)
		return;
	...
	{
		int a = sizeof(hostcache[0].addr);
		memset (&hostcache[0].addr, 0, sizeof(hostcache[0].addr)); // = 0; // Baker hostcache_t
	}
...
qsocket_t *Loop_Connect (const char *host)
{
	...
	if (!loop_client)
	{
		...
		c_strlcpy (loop_client->address, "localhost");
	}
	...
	if (!loop_server)
	{
		...
		c_strlcpy (loop_server->address, "LOCAL");
	}
