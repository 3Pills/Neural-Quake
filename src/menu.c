...
typedef struct
{
	...
	char age[5];
	char kb[5];
	char mapstart[MAX_QPATH];
} demo_item_t;

int m_demos_menu_line_count;
const char *m_demos_cursor_name[MAX_QPATH];
...
const char *m_levels_cursor_name[MAX_QPATH];
...
//	if (!sv_autosave.value)  // ALWAYS exclude autosaves!
	...
	for (i = 0, n = 0 ; i < MAX_SAVEGAMES ; i++)
	{
		...
		if (autosaved)
		{
//			int q = SAVEGAME_COMMENT_LENGTH;
			c_strlcpy (m_filenames[i], "Auto:");
			c_strlcat (m_filenames[i], name);
			m_filenames[i][21] = ' ';
			strlcpy (&m_filenames[i][22], &name[22], SAVEGAME_COMMENT_LENGTH - 22);
		}
...
void M_Preferences_Draw (void)
{
	...
	const char *_weapondraw = r_viewmodel_winquake.value ? "Quake Default" : "FitzQuake Style";
	const char *_invisibility = r_viewmodel_always.value ? "Draw Weapon" : "Quake Default";
...
#else
	const char *_flashblend = "Only applies to GL";
	...
	const char *help_text[] =
	{
		...
		"WinQuake moved weapon up a little",
...
*/
	...
	else if (preferences_cursor < 10)
		cursor_row = 40 + preferences_cursor * 8;
	else
	{
		cursor_col = 12*8;
	...
	M_PrintWhite (22*8 - strlen(help_text[preferences_cursor])*(8/2), i += 8, help_text[preferences_cursor]);
...
void M_Preferences_Key (int k)
{
	...
	case 1: // Weapon
		Cvar_SetValueQuick (&r_viewmodel_winquake, !r_viewmodel_winquake.value);
		break;

	case 2: // Invisibility Weapon
		Cvar_SetValueQuick (&r_viewmodel_always, !r_viewmodel_always.value);
		break;
	...
	case 4: // Bobbing
		{
			...
			default:
				...
				Cvar_SetValueQuick (&r_waterwarp, 0);
				Cvar_ResetQuick (&cl_sidebobbing);
				break;
			}
		}
	...
	case 10: // Set to FitzQuake
		Cvar_ResetQuick (&scr_crosshair);
		Cvar_ResetQuick (&r_viewmodel_winquake);
		Cvar_ResetQuick (&r_viewmodel_always);
		Cvar_ResetQuick (&v_polyblend);
		Cvar_ResetQuick (&v_polyblend_lite);
#ifdef GLQUAKE_FLASH_BLENDS
		...
		break;
	case 11: // Set to Mark V
		Cvar_SetValueQuick (&scr_crosshair, 2);
		Cvar_ResetQuick (&r_viewmodel_winquake);
		Cvar_SetValueQuick (&r_viewmodel_always, 1);
		Cvar_SetValueQuick (&v_polyblend_lite, 1);
		Cvar_ResetQuick (&v_polyblend);
#ifdef GLQUAKE_FLASH_BLENDS
...
//		l = strlen (bindnames[i][0]);
		...
		else
		{
			...
			if (keys[1] != -1)
			{
				...
				M_Print (140 + x + 32, y, Key_KeynumToString (keys[1], key_local_name));
			}
		}
	}
...
void M_GameOptions_Draw (void)
{
	...
	if (pr_fraglimit.value == 0)
		...
		M_Print (160, 88, va("%i frags", (int)pr_fraglimit.value));

	M_Print (0, 96, "       Time Limit");
	if (pr_timelimit.value == 0)
		M_Print (160, 96, "none");
	else
		M_Print (160, 96, va("%i minutes", (int)pr_timelimit.value));
