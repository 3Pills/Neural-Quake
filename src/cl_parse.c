...
char *VersionString (void)
{
	...
	c_snprintf3 (str, "%s %s %g", PLATFORM_SHORTNAME, RENDERER_NAME, ENGINE_VERSION);
...
static void CL_Hint_Set (int cl_new_hintnum, const char *cl_set_hintstring)
{
	...
	case hint_game:
		...
		Con_DPrintf ("Game change request: cls.signon is %d\n", cls.signon);
		result = Host_Gamedir_Change (stringbufx, arg2, true, (const char **)&str);
		if (result == -1 /*fail*/)
		{
	...
	default:
		...
		Con_DPrintf ("Unknown hintnum %d from %s", cl_new_hintnum, hintnames[cl_new_hintnum].keystring );
		return;
	}
