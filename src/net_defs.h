...
typedef struct qsocket_s
{
	...
	byte			proquake_flags;
	int				client_port;
	cbool			net_wait;		// JPG 3.40 - wait for the client to send a packet to the private port
...
typedef struct
{
	...
	int 			(*GetNameFromAddr) (struct qsockaddr *addr, char *name, size_t len); // Baker: Fix by adding maxsize?
...
typedef struct
{
	...
	int				(*Init) (void);
	void			(*Listen) (cbool state);
	void			(*SearchForHosts) (cbool xmit);
