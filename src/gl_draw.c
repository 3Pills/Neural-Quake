...
qpic_t *Draw_PicFromWad (const char *name)
{
	...
	do
	{
		...
		if (gl_external_textures.value)
		{
			...
			data = Image_Load (current_filename, &fwidth, &fheight);//, mod->loadinfo.searchpath);
			if (data)
			{
...
//
	...
	do
	{
		...
		if (gl_external_textures.value)
		{
			...
			if (data)
			{
				gl.gltexture = TexMgr_LoadImage (NULL, -1 /*not bsp texture*/, path, fwidth, fheight, SRC_RGBA, data, path,
											  0, TEXPREF_ALPHA | /* Baker: Crisp -> */ TEXPREF_NEAREST | TEXPREF_PAD | TEXPREF_NOPICMIP); //johnfitz -- TexMgr
				gl.sl = gl.tl = 0;
				gl.sh = gl.th = 1;
				Hunk_FreeToLowMark (mark); // Should be no files to close, LoadImage closes them.
...
//if (newcanvas == CANVAS_MENU_INTERMISSION_TEXT ) newcanvas = CANVAS_MENU;
	...
	case CANVAS_MENU:
		...
		eglViewport (clx + (clwidth - 320*s) / 2, cly + (clheight - 200*s) / 2, 320*s, 200*s);
		break;
